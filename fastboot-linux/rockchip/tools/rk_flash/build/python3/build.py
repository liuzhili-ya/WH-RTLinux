...
class Build:
    ...
    def build_uboot(self):
        ...
        if secureboot_enable:
            ...
            #wanghh cmd = './make.sh %s' % uboot_config
            cmd = './make.sh %s --spl-new' % uboot_config
    ...
    def build_boot_linux(self):
        ...
        #wanghh cmd = 'genext2fs -B %d -b %d -d %s/boot_linux -i 8192 -U %s/boot_linux.img' % (blocks, block_size, kernel_path, kernel_path)
        cmd = 'genext2fs -b %d -d %s/boot_linux -i 8192 -U %s/boot_linux.img' % (3 * block_size, kernel_path, kernel_path)
        if edge_cmd(cmd, None) != 0:
